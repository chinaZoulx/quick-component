...
object HttpService {
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    if (builder.downloadEndIndex == builder.downloadStartIndex) {/*本地与线上一致*/
                        QuickAsync.runOnUiThread {
                            onProgressListener.onResponse(File(config.cachePath + File.separatorChar + HttpUtils.getFileName(builder.url)))
                            onProgressListener.onEnd()
                        }
                    } else
                        downloadPost(builder, onProgressListener)
                }
            })
        else downloadPost(builder, onProgressListener)
    }
     ...
     * 使用GET方式下载文件
     */
    private fun downloadGet(builder: Builder, onProgressListener: OnProgressListener) {
        val request = getRequest(builder).build()
     ...
     * 使用POST方式下载文件
     */
    private fun downloadPost(builder: Builder, onProgressListener: OnProgressListener) {
    ...
    class Builder(val url: String) {
         ...
         * 下载文件，可协带参数
         */
        fun download(onProgressListener: OnProgressListener) {
         ...
         */
        fun downloadBreakpoint(onProgressListener: OnProgressListener) {
            isDownloadBreakpoint = true
            if (downloadStartIndex == 0L) downloadStartIndex = getLocalDownloadLength(this)

            HttpService.downloadBreakpoint(this, onProgressListener)/*断点下载*/
        }
    }
