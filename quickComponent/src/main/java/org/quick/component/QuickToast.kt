...
open class QuickToast {
    ...
    private lateinit var holder: QuickViewHolder
    ...
    private fun configToast(msg: String?): Toast {
        if (toast == null || (builder.layoutView != null && toast!!.view != builder.layoutView)) {/*布局发生变化将重新初始化*/
            toast = Toast(context)
            holder = if (builder.layoutView != null) QuickViewHolder(builder.layoutView!!) else QuickViewHolder((context.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater).inflate(R.layout.app_toast, null))
